...
MATCH (author:Author{name:'Frank Herbert'})-[:WRITES]->(books)
RETURN author, books
...
RETURN customer, orders, sum(orders.total)
...
Return  genre.name ,avg(b.pageCount) as PageCount

// Query #6 Categories that have no sub-categories
MATCH (category:Category) WHERE NOT (category) - [:HAS] -> ()
RETURN category
...
RETURN sum(c.quantity) as SumOfQuantity, b.name
